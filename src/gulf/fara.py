...
def assess_fara_requirement(entity: dict, foreign_payments: list) -> dict:
    ...
    return emit_receipt("fara_assessment", {
        "tenant_id": TENANT_ID,
...
def check_registration(entity_id: str, registered: bool = False,
                       registration_date: str = None) -> dict:
    ...
    Returns:
        fara_check_receipt
    """
    return emit_receipt("fara_check", {
        "tenant_id": TENANT_ID,
        "entity_id": entity_id,
...
def flag_violation(entity_id: str, evidence: dict) -> dict:
    ...
    severity = "critical" if evidence.get("government_payments", 0) > 10_000_000 else \
               "high" if evidence.get("government_payments", 0) > 1_000_000 else "medium"

    return emit_receipt("fara_violation", {
        "tenant_id": TENANT_ID,
        "entity_id": entity_id,
        "evidence": evidence,
        ...
        "severity": severity,
        "special_counsel_requested": evidence.get("special_counsel_requested", False),
